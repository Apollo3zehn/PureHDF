...
internal class H5NativeGroup : H5AttributableObject, IH5NativeGroup
{
    ...
    public Task<IH5Group> GroupAsync(string path, H5LinkAccess linkAccess, CancellationToken cancellationToken = default)
    {
        return Task.FromResult(Group(path, linkAccess));
    }

    public IH5Dataset Dataset(string path)
    {
        return Dataset(path, default);
    }

    public Task<IH5Dataset> DatasetAsync(string path, CancellationToken cancellationToken = default)
    {
        return Task.FromResult(Dataset(path));
    }

    public IH5Dataset Dataset(string path, H5LinkAccess linkAccess = default)
    {
        var link = Get(path, linkAccess);
    ...
    public Task<IH5Dataset> DatasetAsync(string path, H5LinkAccess linkAccess, CancellationToken cancellationToken = default)
    {
        return Task.FromResult(Dataset(path, linkAccess));
    }

    public IH5CommitedDatatype CommitedDatatype(string path)
    {
        return CommitedDatatype(path, default);
    }

    public Task<IH5CommitedDatatype> CommitedDatatypeAsync(string path, CancellationToken cancellationToken = default)
    {
        return Task.FromResult(CommitedDatatype(path));
    }

    public IH5CommitedDatatype CommitedDatatype(string path, H5LinkAccess linkAccess = default)
    {
        var link = Get(path, linkAccess);
    ...
    public Task<IH5CommitedDatatype> CommitedDatatypeAsync(string path, H5LinkAccess linkAccess, CancellationToken cancellationToken = default)
    {
        return Task.FromResult(CommitedDatatype(path, linkAccess));
    }
