...
///     <see cref="Memory{T}" /> that a <c>stackalloc</c>'d <see cref="Span{T}" /> cannot provide
///     (a Span cannot cross an <c>await</c>).
/// </summary>
/// <remarks>
...
///     call - about 32 bytes per rent, which on a scalar read is the entire measured allocation.
///     <see cref="ArrayPool{T}" /> hands back the array itself and allocates nothing,
