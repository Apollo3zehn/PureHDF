...
internal static class H5Cache
{
    ...
    static H5Cache()
    {
        ...
        _fileMap = new ConcurrentDictionary<Superblock, Dictionary<string, NativeH5File>>();
    }
    ...
    private static readonly ConcurrentDictionary<Superblock, Dictionary<string, NativeH5File>> _fileMap;

    public static NativeH5File GetH5File(Superblock superblock, string absoluteFilePath, bool useAsync)
    {
        ...
        {
            pathToH5FileMap = new Dictionary<string, NativeH5File>();
        ...
        if (!pathToH5FileMap.TryGetValue(uri.AbsoluteUri, out var h5File))
        {
            ...
            h5File = (NativeH5File)H5File.Open(uri.LocalPath, FileMode.Open, FileAccess.Read, FileShare.Read, useAsync: useAsync);
